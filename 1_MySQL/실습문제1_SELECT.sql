...
WHERE price BETWEEN 2 AND 4 AND category IN ('Documentary', 'Animation')
		AND actors LIKE '%BOB%';

-- 5. address 테이블에서 district가 비어있지 않고 앞에 숫자 제외 주소만 10개 조회
SELECT substr(address, instr(address, ' ') + 1) address, district ''-- substr의 마지막 추출할 글자 수를 제외하면 끝까지 출력해줌
FROM address
WHERE district != ""
ORDER BY 2, 1 DESC
LIMIT 0, 10;
